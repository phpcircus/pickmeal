<template>
    <div class="w-full relative">
        <text-input v-model="input"
                    ...
                    class="w-full mt-px mr-4 mb-4"
                    placeholder="123 Easy St. Beverly Hills, CA 90210"
                    :label="label"
                    @input="found = false"
        />
        <div v-if="$page.autocomplete.length > 0"
             class="flex flex-col bg-white border border-gray-500 absolute top-16 w-full p-2 mb-4 z-20"
        >
            <div v-for="suggestion in $page.autocomplete"
                 :key="suggestion.locationId"
                 class="w-full mb-1 px-2 py-2 border-b border-gray-400 cursor-pointer last:mb-0 hover:bg-blue-500 group"
            >
                <span class="block text-sm text-gray-700 font-semibold whitespace-no-wrap overflow-hidden group-hover:text-white"
...
export default {
    ...
    methods: {
        ...
        setAddress (suggestion) {
            ...
            this.$dispatch('locationSet', { id: suggestion.locationId, address: this.input });
        },
    },
}
</script>
